...
function injectLiveChat() {
  const { liveChatLicense } = useAppConfig();
  onMounted(() => {
    if (document.getElementById("LiveChatWidget")) {
      return;
    }
    const script = document.createElement("script");
    script.setAttribute("id", "LiveChatWidget");
...
function provideLiveChat(): void {
  injectLiveChat();
}

function useLiveChat(): any {
  // @ts-ignore
  return window.LiveChatWidget;
}

export { provideLiveChat, useLiveChat };
